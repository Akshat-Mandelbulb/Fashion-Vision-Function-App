...
OPENAI_DEPLOYMENT_NAME = os.getenv("OPENAI_DEPLOYMENT_NAME") or ""
...
def analyze_fashion_item(image_url, custom_prompt=None):
    ...
    if error == "content_filter":
        logging.warning(
            "Primary analysis blocked by content policy. Trying fallback..."
        )
...
def analyze_image(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        ...
        if error == "content_filter":
            ...
            return func.HttpResponse(
                json.dumps({"error": "Analysis failed", "message": error}),
                status_code=500,
                mimetype="application/json",
            )
